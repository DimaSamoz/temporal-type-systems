...
mutual
    ...
    substₚ Γ Γ′ {C now} M (stable S) rewrite ˢ-pres-⌊⌋ (Γ , C now) Γ′
                                         | sym (ˢ-pres-⌊⌋ Γ Γ′)
                                         = stable S
    substₚ Γ Γ′ {C always} M (stable {A = A} S)
        rewrite ˢ-pres-⌊⌋ (Γ , C always) Γ′
            = stable S′
        where
        M′ : Γ ˢ ⌊⌋ Γ′ ˢ ⊢ C always
        M′ rewrite sym (ˢ-pres-⌊⌋ Γ Γ′) = ˢ-always M
        S′ : (Γ ⌊⌋ Γ′) ˢ ⊢ A now
        S′ rewrite ˢ-pres-⌊⌋ Γ Γ′ = substₚ (Γ ˢ) (Γ′ ˢ) M′ S
    ...
    substₚᶜ Γ Γ′ {D always} M (letEvt_In_ {A = A} {B} E C)
        rewrite ˢ-pres-⌊⌋ (Γ , D always) Γ′
            = letEvt substₚ Γ Γ′ M E In C′
        where
        M′ : Γ ˢ ⌊⌋ Γ′ ˢ ⊢ D always
        M′ rewrite sym (ˢ-pres-⌊⌋ Γ Γ′) = ˢ-always M
        C′ : (Γ ⌊⌋ Γ′) ˢ , A now ⊨ B now
        C′ rewrite ˢ-pres-⌊⌋ Γ Γ′
    ...
    substᶜ Γ Γ′ {D}{F} (letEvt_In_ {A = A} E M) C
        ...
        C′ : (Γ ⌊⌋ Γ′) ˢ , A now ⊨ F now
        C′ rewrite ˢ-pres-⌊⌋ Γ Γ′ = substᶜ (Γ ˢ) (Γ′ ˢ , A now) M C-idemp
