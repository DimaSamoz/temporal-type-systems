...
module ⟦K⟧ {𝒮} {k : Kit 𝒮} (⟦k⟧ : ⟦Kit⟧ k) where
    ...
    ⟦subst⟧-⟦select⟧ {Δ} A B σ n l (inj₁ (inj₁ (⟦A⟧ , ⟦◇B⟧))) ⟦Δ⟧
        rewrite ⟦↑⟧ (A now) (_↑_ {Event B now} (σ ↓ˢ k) k) {l} {(⟦ Δ ˢ⟧□ n ⟦Δ⟧ l , ⟦◇B⟧) , ⟦A⟧}
              | ⟦↑⟧ (Event B now) (σ ↓ˢ k) {l} {⟦ Δ ˢ⟧□ n ⟦Δ⟧ l , ⟦◇B⟧}
              | □-≡ n l (⟦↓ˢ⟧ σ {n} {⟦Δ⟧}) l = refl
    ⟦subst⟧-⟦select⟧ {Δ} A B σ n l (inj₁ (inj₂ (⟦◇A⟧ , ⟦B⟧))) ⟦Δ⟧
        rewrite ⟦↑⟧ (B now) (_↑_ {Event A now} (σ ↓ˢ k) k) {l} {(⟦ Δ ˢ⟧□ n ⟦Δ⟧ l , ⟦◇A⟧) , ⟦B⟧}
              | ⟦↑⟧ (Event A now) (σ ↓ˢ k) {l} {⟦ Δ ˢ⟧□ n ⟦Δ⟧ l , ⟦◇A⟧}
              | □-≡ n l (⟦↓ˢ⟧ σ {n} {⟦Δ⟧}) l = refl
    ⟦subst⟧-⟦select⟧ {Δ} A B σ n l (inj₂ (⟦A⟧ , ⟦B⟧)) ⟦Δ⟧
        rewrite ⟦↑⟧ (B now) (_↑_ {A now} (σ ↓ˢ k) k) {l} {(⟦ Δ ˢ⟧□ n ⟦Δ⟧ l , ⟦A⟧) , ⟦B⟧}
              | ⟦↑⟧ (A now) (σ ↓ˢ k) {l} {⟦ Δ ˢ⟧□ n ⟦Δ⟧ l , ⟦A⟧}
              | □-≡ n l (⟦↓ˢ⟧ σ {n} {⟦Δ⟧}) l = refl
