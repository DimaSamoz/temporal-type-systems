...
subst″-sound {Γ}{A}{B} (select_↦_||_↦_||both↦_ {A = G}{H} E₁ C₁ E₂ C₂ C₃) D {n} {⟦Γ⟧} =
    ...
    -- ≡⟨ cong! (ext λ m → ext λ b → substitute′-sound ((_⁺_ {G always} (idₛ 𝒯erm) 𝒯erm) ↑ 𝒯erm) D {m} {b}) ⟩
    --     bindEvent (Γ ,, G always) ⟦ C ⟧ᵐ
    --         (⟦ D ⟧ᵐ ∘ ⌞ ⟦ _↑_ {A now} {Γ = Γ ˢ} (_⁺_ {G always} (idₛ 𝒯erm) 𝒯erm) 𝒯erm ⟧ₛ ⌟) n (⟦Γ⟧ , ⟦ S ⟧ₘ n ⟦Γ⟧)
    -- ≡⟨ cong! (ext λ m → ext λ b → ⟦↑⟧ (A now) {Γ = Γ ˢ}(_⁺_ {G always} (idₛ 𝒯erm) 𝒯erm) {m} {b}) ⟩
    --     bindEvent (Γ ,, G always) ⟦ C ⟧ᵐ
    --         (⟦ D ⟧ᵐ ∘ ⌞ ⟦ _⁺_ {G always} {Γ = Γ ˢ} (idₛ 𝒯erm) 𝒯erm ⟧ₛ ⌟ * id) n (⟦Γ⟧ , ⟦ S ⟧ₘ n ⟦Γ⟧)
    -- ≡⟨ cong! (ext λ m → ext λ b → ⟦⁺⟧ (G always) (idₛ {Γ ˢ} 𝒯erm) {m} {b}) ⟩
    --     bindEvent (Γ ,, G always) ⟦ C ⟧ᵐ
    --         (⟦ D ⟧ᵐ ∘ (⌞ ⟦ idₛ {Γ ˢ} 𝒯erm ⟧ₛ ⌟ ∘ π₁) * id) n (⟦Γ⟧ , ⟦ S ⟧ₘ n ⟦Γ⟧)
    -- ≡⟨ cong! (ext λ m → ext λ b → ⟦idₛ⟧ {Γ = Γ ˢ} {m} {b}) ⟩
    --     bindEvent (Γ ,, G always) ⟦ C ⟧ᵐ (⟦ D ⟧ᵐ ∘ π₁ * id) n (⟦Γ⟧ , ⟦ S ⟧ₘ n ⟦Γ⟧)
    -- ≡⟨⟩
    --     μ.at ⟦ B ⟧ₜ n (◇-f (⟦ D ⟧ᵐ ∘ π₁ * id) n
    --         (◇-f (ε.at (⟦ Γ ˢ ⟧ₓ ⊗ □ ⟦ G ⟧ₜ) * id) n
    --         (st (⟦ Γ ˢ ⟧ₓ ⊗ □ ⟦ G ⟧ₜ) ⟦ A ⟧ₜ n
    --             ( (⟦ Γ ,, G always ˢ⟧□) n (⟦Γ⟧ , ⟦ S ⟧ₘ n ⟦Γ⟧)
    --             , ⟦ C ⟧ᵐ n (⟦Γ⟧ , ⟦ S ⟧ₘ n ⟦Γ⟧)))))
    -- ≡⟨ cong (μ.at ⟦ B ⟧ₜ n) (◇-f-∘ {g = ⟦ D ⟧ᵐ} {π₁ * id} {n}
    --                         {◇-f (ε.at (⟦ Γ ˢ ⟧ₓ ⊗ □ ⟦ G ⟧ₜ) * id) n
    --                         (st (⟦ Γ ˢ ⟧ₓ ⊗ □ ⟦ G ⟧ₜ) ⟦ A ⟧ₜ n
    --                             ( (⟦ Γ ,, G always ˢ⟧□) n (⟦Γ⟧ , ⟦ S ⟧ₘ n ⟦Γ⟧)
    --                             , ⟦ C ⟧ᵐ n (⟦Γ⟧ , ⟦ S ⟧ₘ n ⟦Γ⟧)))}) ⟩
...
--     ∎
    ...
    -- rewrite (ext λ m → ext λ b → subst″-sound C (substitute′ ((Γ ˢˢₛ 𝒯erm) ↑ 𝒯erm) D) {m} {b})
    --       | (ext λ m → ext λ b → substitute′-sound ((Γ ˢˢₛ 𝒯erm) ↑ 𝒯erm) D {m} {b}) = {!   !}
    -- begin
    --     bindEvent Γ ⟦ E ⟧ₘ
    --         (bindEvent (Γ ˢ ,, G now) ⟦ C ⟧ᵐ (⟦ D ⟧ᵐ ∘ ⌞ ⟦ _↑_ {A now} (Γ ˢˢₛ 𝒯erm) 𝒯erm ⟧ₛ ⌟)) n ⟦Γ⟧
    -- ≡⟨ cong! (ext λ m → ext λ b → ⟦↑⟧ (A now) (Γ ˢˢₛ 𝒯erm) {m} {b}) ⟩
    --     bindEvent Γ ⟦ E ⟧ₘ
    --         (bindEvent (Γ ˢ ,, G now) ⟦ C ⟧ᵐ (⟦ D ⟧ᵐ ∘ ⟦ Γ ˢˢₛ 𝒯erm ⟧ₛ * id)) n ⟦Γ⟧
    -- ≡⟨ bind-assoc Γ {A} {B} {G} ⟦ E ⟧ₘ ⟦ C ⟧ᵐ ⟦ D ⟧ᵐ ⟩
    --     bindEvent Γ (bindEvent Γ ⟦ E ⟧ₘ ⟦ C ⟧ᵐ) ⟦ D ⟧ᵐ n ⟦Γ⟧
    -- ∎
    ...
    -- ≡⟨⟩
    --     bindEvent Γ ⟪ ⟦ E₁ ⟧ₘ , ⟦ E₂ ⟧ₘ ⟫
    --         (handle ⟦ ⟨ C₁ /⟩ (substitute′ ((Γ ˢˢₛ 𝒯erm) ↑ 𝒯erm) D) ⟧ᵐ
    --                 ⟦ ⟨ C₂ /⟩ (substitute′ ((Γ ˢˢₛ 𝒯erm) ↑ 𝒯erm) D) ⟧ᵐ
    --                 ⟦ ⟨ C₃ /⟩ (substitute′ ((Γ ˢˢₛ 𝒯erm) ↑ 𝒯erm) D) ⟧ᵐ) n ⟦Γ⟧
    -- ≡⟨ {!   !} ⟩
    --     bindEvent Γ ⟪ ⟦ E₁ ⟧ₘ , ⟦ E₂ ⟧ₘ ⟫
    --         (bindEvent {!   !} (handle ⟦ C₁ ⟧ᵐ ⟦ C₂ ⟧ᵐ ⟦ C₃ ⟧ᵐ) (⟦ D ⟧ᵐ ∘ (ε.at {!   !} ∘ ⟦ {!   !} ˢ⟧□) * id)) n ⟦Γ⟧
    -- ≡⟨ {!   !} ⟩
    --     bindEvent Γ (bindEvent Γ ⟪ ⟦ E₁ ⟧ₘ , ⟦ E₂ ⟧ₘ ⟫ (handle ⟦ C₁ ⟧ᵐ ⟦ C₂ ⟧ᵐ ⟦ C₃ ⟧ᵐ)) ⟦ D ⟧ᵐ n ⟦Γ⟧
    -- ∎
    -- where
    ...
    --            ⟦ ⟨ C₁ /⟩ (substitute′ ((Γ ˢˢₛ 𝒯erm) ↑ 𝒯erm) D) ⟧ᵐ
    --            ⟦ ⟨ C₂ /⟩ (substitute′ ((Γ ˢˢₛ 𝒯erm) ↑ 𝒯erm) D) ⟧ᵐ
    --            ⟦ ⟨ C₃ /⟩ (substitute′ ((Γ ˢˢₛ 𝒯erm) ↑ 𝒯erm) D) ⟧ᵐ m b
    --         ≡ ({!   !} >>= λ l ⟦A⟧ → ⟦ D ⟧ᵐ l ((⟦ Γ ˢ⟧□ l {! ⟦Γ⟧  !} {!   !}) , ⟦A⟧))
    --         -- ≈ bindEvent {! Γ  !} (handle ⟦ C₁ ⟧ᵐ ⟦ C₂ ⟧ᵐ ⟦ C₃ ⟧ᵐ) ⟦ D ⟧ᵐ
