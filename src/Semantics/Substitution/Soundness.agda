...
subst″-sound-lemma Γ {A} n k l D ⟦Γ⟧ ⟦A⟧
    rewrite substitute′-sound ((Γ ˢˢₛ 𝒯erm) ↑ 𝒯erm) D {l} {⟦ Γ ˢ ⟧ˢₓ-□ k (⟦ Γ ⟧ˢₓ-□ n ⟦Γ⟧ k) l , ⟦A⟧}
          | ⟦↑⟧ (A now) (Γ ˢˢₛ 𝒯erm) {l} {⟦ Γ ˢ ⟧ˢₓ-□ k (⟦ Γ ⟧ˢₓ-□ n ⟦Γ⟧ k) l , ⟦A⟧}
          | □-≡ k l (□-≡ n k (⟦⟧ˢₓ-□-twice Γ {n} {⟦Γ⟧}) k) l
          | □-≡ k l (⟦ˢˢ⟧ Γ {n} {⟦Γ⟧}) l = refl
...
subst″-sound {Γ} {A} (letSig_InC_ {A = B} S C) D n ⟦Γ⟧ =
    ...
    ≡⟨ cong (λ x → (⟦ C ⟧ᵐ n (⟦Γ⟧ , ⟦ S ⟧ₘ n ⟦Γ⟧) >>= x))
        (ext λ l → ext λ ⟦A⟧ →
            ...
            ≡⟨ cong (⟦ D ⟧ᵐ l) (⟦↑⟧ (A now) {Γ ˢ ,, B always} (_⁺_ {B always} (idₛ 𝒯erm) 𝒯erm) {l} {(⟦ Γ ⟧ˢₓ-□ n ⟦Γ⟧ l , ⟦ S ⟧ₘ n ⟦Γ⟧) , ⟦A⟧}) ⟩
                ⟦ D ⟧ᵐ l (⟦ _⁺_ {B always} {Γ = Γ ˢ} (idₛ 𝒯erm) 𝒯erm ⟧ₛ l (⟦ Γ ⟧ˢₓ-□ n ⟦Γ⟧ l , ⟦ S ⟧ₘ n ⟦Γ⟧) , ⟦A⟧)
            ≡⟨ cong (λ x → ⟦ D ⟧ᵐ l (x , ⟦A⟧)) (⟦⁺⟧ (B always) {Γ ˢ} (idₛ 𝒯erm) {l} {⟦ Γ ⟧ˢₓ-□ n ⟦Γ⟧ l , ⟦ S ⟧ₘ n ⟦Γ⟧}) ⟩
                ⟦ D ⟧ᵐ l (⟦ idₛ {Γ ˢ} 𝒯erm ⟧ₛ l (⟦ Γ ⟧ˢₓ-□ n ⟦Γ⟧ l) , ⟦A⟧)
            ≡⟨ cong (λ x → ⟦ D ⟧ᵐ l (x , ⟦A⟧)) (⟦idₛ⟧ {Γ ˢ} {l} {⟦ Γ ⟧ˢₓ-□ n ⟦Γ⟧ l}) ⟩
                ⟦ D ⟧ᵐ l (⟦ Γ ⟧ˢₓ-□ n ⟦Γ⟧ l , ⟦A⟧)
            ∎
        )
     ⟩
